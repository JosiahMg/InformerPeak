...
class TDOperatedByClient:
    ...
    def query_table_cols(self, table_name):
        """获取表字段名称"""
        return [item[0] for item in self.client.sql(f"""Describe {TDengineConfig['db_name']}.{table_name};""")["data_loader"]]
    ...
    def duplicate_table_struct(self, origin_table, table_name):
        ...
        tags_list = []
        fields_list = []
        struct = self.client.sql(f"""Describe {TDengineConfig['db_name']}.{origin_table};""")["data_loader"]
        for item in struct:
            if 'TAG' in item:
    ...
    def _row2df(_res):
        ...
        try:
            ...
            return pd.DataFrame(data=_res['data_loader'], columns=cols)
        except Exception as e:
